...
async function verify(client: PrismaClient, identity: Awaited<ReturnType<typeof initialize>>) {
  ...
  await assert.rejects(client.$transaction(async tx => {
    ...
    await tx.stagingInventory.create({ data: { mrpRunId: run.id, sourceRecordId: rollbackCode, erpPartNo: rollbackCode, goodStockPc: 123 } });
  ...
  assert.equal(await client.stagingInventory.count({ where: { sourceRecordId: rollbackCode } }), 0, 'POSTGRES_TRANSACTION_ROLLBACK');
