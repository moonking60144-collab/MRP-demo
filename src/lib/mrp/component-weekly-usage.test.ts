...
test('明細 movement 狀態能區分 Form 20、Form 28 混合來源', () => {
...
test('未結案工令的 Form 20 OUT 保持帳面出庫待結算，不宣稱為已確認實耗', () => {
...
test('相同 Form 20 OUT 依工令狀態區分未結案待結算與已結案已結算', () => {
  const quantities = {
...
test('工令用料明細只加總同料號同週且剩餘量大於零的工令', () => {
  const result = summarizeComponentWeeklyUsage({
    ...
    bomRows: [
      bomRow(),
      bomRow({ ragicRecordId: '2', woNumber: 'WO-002', alreadyPicked: 'No', issuedQty: 0, remainingUsage: 600, issuedQtyState: 'not_issued' }),
      bomRow({ ragicRecordId: '3', woNumber: 'WO-003', issuedQty: 600, remainingUsage: 0 }),
      bomRow({ ragicRecordId: '4', woNumber: 'WO-004', issuedQty: null, remainingUsage: null, issuedQtyState: 'unknown', issuedQtyError: 'missing_issue_details' }),
      bomRow({ ragicRecordId: '5', woNumber: 'WO-005', startDate: new Date('2026-07-22T00:00:00.000Z') }),
      bomRow({ ragicRecordId: '6', componentNo: 'WIRE-02' }),
    ],
    workOrders: [
      { ragicRecordId: 'wo-1', woNumber: 'WO-001', erpPartNo: 'FG-01-V01', status: '未結案' },
      { ragicRecordId: 'wo-2', woNumber: 'WO-002', erpPartNo: 'FG-02-V01', status: '未結案' },
    ],
  });
  ...
  assert.equal(result.included[0]?.workOrderRagicRecordId, 'wo-1');
...
test('工令用料明細與週推移共用 Form 96 指定開始日，並揭露 Form 28 日期差異', () => {
  const result = summarizeComponentWeeklyUsage({
    materialPartNo: 'WIRE-01',
    weekIndex: 2,
    weeks,
...
test('W 工令明細排除與 canonical unit 不同的 BOM，並明確回傳單位異常', () => {
  const result = summarizeComponentWeeklyUsage({
    ...
    bomRows: [
      bomRow({ ragicRecordId: 'kg-row', unit: 'kg', remainingUsage: 500 }),
      bomRow({ ragicRecordId: 'pc-row', woNumber: 'WO-PC', unit: 'pc', remainingUsage: 537 }),
    ],
    workOrders: [],
  });

  assert.equal(result.included.length, 1);
  assert.equal(result.included[0]?.ragicRecordId, 'kg-row');
  assert.equal(result.includedTotal, 500);
  assert.equal(result.unitMismatchCount, 1);
  assert.equal(result.excluded.find((row) => row.ragicRecordId === 'pc-row')?.status, 'unknown');
  assert.equal(result.excluded.find((row) => row.ragicRecordId === 'pc-row')?.reason, 'unit_mismatch');
  assert.equal(result.reconciled, true);
});
...
test('重複使用批號的 Form 20 快照不虛構退料，未結案 OUT 標示為待結算帳面出庫', () => {
  const result = summarizeComponentWeeklyUsage({
    materialPartNo: 'WIRE-01',
    weekIndex: 1,
    weeks,
...
test('Form 20 未確認時不以 Form 28 舊領用量偽造淨領用與正式退料', () => {
  const result = summarizeComponentWeeklyUsage({
    materialPartNo: 'WIRE-01',
    weekIndex: 1,
    weeks,
