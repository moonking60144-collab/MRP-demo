...
function orderedMovements(
  ...
  return [...movements].sort((left, right) => {
    ...
    const idDiff = recordOrder(left.ragicRecordId) - recordOrder(right.ragicRecordId);
    return idDiff !== 0
      ? idDiff
      : left.ragicRecordId.localeCompare(right.ragicRecordId);
  });
}
...
export function resolveWorkOrderMaterialLedgers(input: {
  ...
  const attribute = (movement: WorkOrderLedgerMovement, bomRecordId: string) => {
    attributedBomRecordIds.set(movement.ragicRecordId, bomRecordId);
    const summary = summaries.get(bomRecordId)!;
    if (!summary.movementRecordIds.includes(movement.ragicRecordId)) {
      summary.movementRecordIds.push(movement.ragicRecordId);
    }
  };
