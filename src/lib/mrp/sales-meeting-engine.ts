 ...
 * Port of calculateMrpSalesMeeting() from Source JS (synthetic planning reference workflow, lines 1-768)
...
// MUST stay at 12 to match Source's d4/36 workflow exactly:
//   - Source loops `for (i = 1; i <= 12; i++)` over `weeklyPeriods[i-1]` (only 12 periods used)
//   - avgDemandPerWeek divisor below is hard-coded to (12 + 1) = 13 (prior + 12 weeks)
// Earlier this was 13 which produced an extra W13 column the Source export
  ...
  for (const [, group] of byErp) {
    ...
    for (const part of group) {
      ...
      summaryBatch.push({
        ...
        mainStockPc: inv?.mainStockPc ?? null,
        auxStockPc: inv?.auxStockPc ?? null,
...
async function loadInventory(
  ...
  for (const row of rows) {
    ...
    map.set(cleanErp, {
      ...
      mainStockPc: row.mainStockPc == null ? null : Number(row.mainStockPc),
      auxStockPc: row.auxStockPc == null ? null : Number(row.auxStockPc),
...
async function loadOrderDemand(
  ...
  for (const row of rows) {
    // 週分桶用「營業指定出貨日」— 對齊 Source（與 fg-monthly 月分桶一致）
...
function dateToWeekLabel(date: Date, weeks: MrpWeek[]): string {
  ...
  // demand map. Source stores demand by ISO yearWeek which gives the same
