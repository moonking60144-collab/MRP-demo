...
test('廠內與 YE1 是獨立倉庫區段，簡易表排在期末無計劃剩餘前', () => {
  assert.deepEqual(FG_TRAD_WAREHOUSE.map((column) => column.key), ['wfgStockPc', 'ye1StockPc']);
  assert.equal(FG_TRAD_STATIC.some((column) => column.key === 'wfgStockPc' || column.key === 'ye1StockPc'), false);

  const ids = FG_MONTHLY_SIMPLE_COLUMNS.map((column) => column.id);
  const wfgIndex = ids.indexOf('wfgStockPc');
  const ye1Index = ids.indexOf('ye1StockPc');
  const remainingIndex = ids.indexOf('lastPeriodRemainingNoPlan');
  assert.equal(ye1Index, wfgIndex + 1);
  assert.equal(remainingIndex, ye1Index + 1);
});

test('hidden-only 偏好讓新欄預設顯示並保留另一個 view 的隱藏項', () => {
  const merged = mergeColumnVisibility(
    { currentStockPc: true, wfgStockPc: true, ye1StockPc: true },
  ...
  const hidden = buildHiddenColumnPreference(
    ['currentStockPc', 'wfgStockPc', 'ye1StockPc'],
    { currentStockPc: true, wfgStockPc: false, ye1StockPc: true },
