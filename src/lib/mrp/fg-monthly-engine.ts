...
interface PartProjectionInputs {
  ...
  numPeriods: number;
  baseDate: Date;
  /** true 時把「不計算成品庫存」料件的成品庫存當 0;false 照算真實庫存（仿 Ragic）。 */
 ...
 * Mirrors Ragic d4/21 calculateMrpMonthlyProjection2, with invalid negative shipped quantity clamped.
 * Caller owns the I/O.
 */
export function computePartProjection(opts: PartProjectionInputs): PartProjectionResult {
  ...
  // WO HF01 split — Ragic d4/21:2197-2205
  let totalHf01 = 0;
  ...
  for (const order of orders) {
    ...
    // 月份分桶用「營業指定出貨日」— 對齊 Ragic d4/21（非客戶訂單需求日）
  ...
  // 對齊 Ragic：前期計畫產出(priorPlanOutput)是 Ragic 的「第0期」，不併進第1期。
  ...
  // Running stock — Ragic d4/21:2457-2469
  ...
  // 缺預示期數 — Ragic d4/21:2484-2491 (loop `i <= stockingPeriods` with floating)
...
export async function calculateFgMonthly(
  ...
  // Use the run's runDate (matches Ragic d4/21: monthlyPeriods built from triggerEntry runDate)
  const run = inputs
    ? null
  ...
  for (const part of parts) {
    ...
    const stock = resolveInventoryAvailability({
      ...
      wfgStockPc: inv?.wfgStockPc,
      wfgStockKg: inv?.wfgStockKg,
      ye1StockPc: inv?.ye1StockPc,
      ye1StockKg: inv?.ye1StockKg,
    });
    const rawStock = stock.inStockPc;
    const wfgStock = stock.wfgStockPc;
    const ye1Stock = stock.ye1StockPc;
    ...
    preparedParts.push({ partData, rawStock, wfgStock, ye1Stock, badStock, result });
  }
  ...
  for (const { partData, rawStock, wfgStock, ye1Stock, badStock, result: r } of preparedParts) {
 ...
 *   - Then run the SAME Ragic d4/21 formula on the merged inputs.
 */
export async function calculateFgMonthlyAggregated(
  ...
  for (const p of parts) {
    if (!p.partVersion || !p.erpPartNo) continue; // mirror Ragic d4/21:2293 skip
  ...
  for (const [, members] of groups) {
    ...
    for (const erp of uniqueErps) {
      ...
      aggWfgStock += stock.wfgStockPc;
      aggYe1Stock += stock.ye1StockPc;
    ...
    summaryBatch.push({
      ...
      wfgStockPc: aggWfgStock,
      ye1StockPc: aggYe1Stock,
