...
test('元件需求優先依 Form 96 指定開始日分週，不使用過期的 Form 28 日期', () => {
  const { usageMap } = aggregateComponentWeeklyUsageRows([
    bomRow({
      woNumber: 'WO-26070489',
      startDate: new Date('2026-07-14T00:00:00.000Z'),
      minUsage: 4144,
      issuedQty: 2227,
      remainingUsage: 1917,
    ...
    workOrderStartDates: new Map([
      ['WO-26070489', new Date('2026-07-21T00:00:00.000Z')],
    ]),
  });
...
test('W 線材同料號混用 kg 與 pc 時只納入 canonical unit，異常列不得混算', () => {
  const result = aggregateComponentWeeklyUsageRows([
    bomRow({
      ragicRecordId: 'kg-row',
      startDate: new Date('2026-07-14T00:00:00.000Z'),
      unit: 'kg',
      remainingUsage: 500,
    }),
    bomRow({
      ragicRecordId: 'pc-row',
      startDate: new Date('2026-07-14T00:00:00.000Z'),
      unit: 'pc',
      remainingUsage: 537,
    }),
...
test('W 線材庫存無法判定單位但 BOM 單位唯一時可採用該來源單位', () => {
  const result = aggregateComponentWeeklyUsageRows([
    bomRow({
      ragicRecordId: 'kg-row-1',
      startDate: new Date('2026-07-14T00:00:00.000Z'),
      unit: 'kg',
      remainingUsage: 500,
    }),
    bomRow({
      ragicRecordId: 'kg-row-2',
      startDate: new Date('2026-07-14T00:00:00.000Z'),
      unit: 'KG',
      remainingUsage: 250,
    }),
