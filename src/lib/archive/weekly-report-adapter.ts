...
export function adaptArchiveWeeklyReport(report: ArchiveWeeklyReport) {
  ...
  const items = report.rows.map(row => {
    ...
    return component ? item : { ...item, memberPartVersions: [row.part_version], sharedErpCount: 1,
      ...
      mainInventoryAnomalyCount: 0, auxInventoryAnomalyCount: 0, inventoryAnomalyDiffPc: 0, inventoryAnomalyErpPartNos: [],
      mainStockPc: report.warehouseAvailable ? item.mainStockPc : null, auxStockPc: report.warehouseAvailable ? item.auxStockPc : null };
  });
