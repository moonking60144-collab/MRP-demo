...
test('sales history retains stored part-version rows without current grouping and gates warehouse claims', () => {
  const sales: ArchiveWeeklyReport = { ...report, kind: 'sales', rows: [{ id: '1', mrp_run_id: '134', part_version: '001.10', customer_code: null, customer_part_no: '0001', main_stock_pc: '20', aux_stock_pc: '0', fg_diff_0_4: '-20', fg_status_0_4: 'shortage' }], periods: [] };
  ...
  assert.equal(salesItems[0].mainStockPc, null);
  assert.equal(salesItems[0].auxStockPc, null);
  assert.equal(salesItems[0].inventoryValidationAvailable, false);
  assert.deepEqual(salesItems[0].memberPartVersions, ['001.10']);
  assert.equal(adaptArchiveWeeklyReport({ ...sales, warehouseAvailable: true }).salesItems[0].mainStockPc, 20);
});
...
test('historical source search chooses compatible identities and leaves unknown BOM relation explicit', () => {
  const queries = archiveSourceQueries({ query: 'DEMO-PRODUCT-001', partVersion: 'DEMO-PRODUCT-001', erpPartNo: 'DEMO-ERP-001-V01' });
  assert.equal(queries?.orders, 'DEMO-PRODUCT-001');
  assert.equal(queries?.inventory, 'DEMO-ERP-001-V01');
