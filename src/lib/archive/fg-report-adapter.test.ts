...
const report: ArchiveFgReport = {
  ...
  rows: [{ id: '1', mrp_run_id: '134', part_version: '1.10', is_aggregated: 'false', current_stock_pc: '100', unit_weight_g: null, wfg_stock_pc: '0', ye1_stock_pc: '20', shortage_start_period: null, shared_erp_count: '2' }],
...
test('archive adapter preserves identifiers, nullable metrics, missing month positions and warehouse availability', () => {
  ...
  assert.equal(items[0].wfgStockPc, null);
  assert.equal(items[0].ye1StockPc, null);
  ...
  assert.equal(adaptArchiveFgReport({ ...report, warehouseAvailable: true }).items[0].ye1StockPc, 20);
