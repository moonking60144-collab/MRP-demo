...
export async function readPurchaseLeadTimeResponse(response: Response): Promise<PurchaseLeadTimeResponse> {
  const unknownOutcome = `前置期更新結果無法確認（HTTP ${response.status}）；請先開啟 Ragic 原單核對，確認前不要重送。`;
  ...
  if (!response.ok) {
    const currentValue = result.currentConfigured === false
      ...
      : typeof result.currentPurchaseLeadWeeks === 'number'
        ...
        : null;
    throw new Error(
      `${typeof result.error === 'string' ? result.error : unknownOutcome}${currentValue ? `；Ragic 目前為「${currentValue}」` : ''}`,
    );
  }
  if (
