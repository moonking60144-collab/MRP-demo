...
test('庫存快照保留所有在庫品質，但排除非在庫狀態', () => {
  ...
  assert.equal(normalizeInventoryLot({ ...base, warehouseCode: ' aux ' })?.warehouseCode, 'AUX');
...
test('MRP合計只納入允許品質，再以 AUX 和非 AUX 拆分', () => {
  const summary = summarizeInventoryLots([
    { erpPartNo: 'ERP-A', warehouseCode: 'MAIN', stockStatus: '在庫', qualityStatus: '正常', stockPc: 100, stockKg: 1 },
    { erpPartNo: 'ERP-A', warehouseCode: 'AUX', stockStatus: '在庫', qualityStatus: '待驗', stockPc: 60, stockKg: 0.5 },
    ...
    { erpPartNo: 'ERP-A', warehouseCode: 'MAIN', stockStatus: '在庫', qualityStatus: '不良', stockPc: 999, stockKg: 9 },
  ]).get('ERP-A');
