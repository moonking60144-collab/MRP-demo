...
test('右鍵切換既有排序欄方向時會移到第一順位且不重複', () => {
  const result = prioritizeSortFields([
    { id: 'forgingParent', direction: 'asc', label: '鍛造母件' },
    { id: 'wfgStockPc', direction: 'asc', label: '廠內（非YE1）在庫pc' },
  ], {
    id: 'wfgStockPc',
    direction: 'desc',
    label: '廠內（非YE1）在庫pc',
...
test('合併資料庫的 Prisma Decimal 先正規化再做數值降冪排序', () => {
  const rows = [
    { partVersion: 'A', wfgStockPc: new Prisma.Decimal(992) },
    { partVersion: 'B', wfgStockPc: new Prisma.Decimal(9947) },
    { partVersion: 'C', wfgStockPc: new Prisma.Decimal(12000) },
  ].map((row) => normalizeFgMonthlyDecimals(row));

  const sorted = applySort(rows, [{ key: 'wfgStockPc', dir: 'desc' }]);
  assert.deepEqual(sorted.map((row) => row.wfgStockPc), [12000, 9947, 992]);
});
