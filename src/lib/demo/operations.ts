...
export function suggestions(runId: number, partVersion?: string): DemoRow[] {
  return state().suggestions.filter((row) => row.mrpRunId === runId && (!partVersion || row.partVersion === partVersion)).map((row) => {
    ...
    return { ...row, transferId: transfer?.id ?? null, ragicRecordId: transfer?.ragicRecordId ?? null, ragicPlanNo: transfer?.ragicPlanNo ?? null, ragicUrl: transfer?.ragicUrl ?? null, workOrderStatus: transfer?.workOrderStatus ?? null, workOrderError: transfer?.workOrderError ?? null, workOrderCompletedAt: transfer?.workOrderCompletedAt ?? null };
  });
}
...
function transferRecord(suggestion: DemoRow): DemoRow {
  ...
  return modelRow('ProductionPlanTransfer', { id, mrpRunId: suggestion.mrpRunId, mrpVersionCode: latestRun().versionCode, partVersion: suggestion.partVersion, planSequence: suggestion.planSequence, customerCode: dataset(latestRun().id).fg.find((row) => row.partVersion === suggestion.partVersion)?.customerCode, suggestedQty: suggestion.suggestedQty, completionDate: suggestion.completionDate, ragicRecordId: String(100000 + id), ragicPlanNo: `DEMO-PLAN-${id}`, ragicUrl: `/demo-record?type=production_plan&id=${100000 + id}`, transferredAt: new Date().toISOString(), transferredBy: 'Demo', workOrderStatus: 'idle' });
}
...
export function reconcile(partVersion: string, body: DemoRow): unknown {
  ...
  const candidates: TransferReconciliationCandidate[] = state().transfers.filter((row) => row.mrpRunId === runId && row.partVersion === partVersion && row.planSequence === sequence).map((row) => ({ ragicRecordId: String(row.ragicRecordId), ragicPlanNo: row.ragicPlanNo as string | null, ragicUrl: String(row.ragicUrl), suggestedQty: Number(row.suggestedQty), completionDate: String(row.completionDate).slice(0, 10), createdAt: String(row.transferredAt), mrpSourceCode: String(row.mrpVersionCode) }));
  ...
  if (body.action === 'confirm_created') {
    const candidate = candidates.find((row) => row.ragicRecordId === String(body.ragicRecordId));
  ...
  if (body.action === 'confirm_not_created') {
    ...
    return { reconciliation: { action: body.action, transferStatus: 'failed', ragicRecordId: null, ragicPlanNo: null, ragicUrl: null } };
  }
...
export function workOrderCommand(transferId: number, action: string, body: DemoRow): unknown {
  ...
  if (action === 'record-link') {
    if (String(body.ragicRecordId) !== String(100000 + transferId)) throw new DemoError(`Demo 只接受此計畫的合成 ID：${100000 + transferId}`);
    const linked = mutate((draft) => { const row = draft.transfers.find((item) => item.id === transferId)!; row.ragicRecordId = String(body.ragicRecordId); row.ragicUrl = `/demo-record?type=production_plan&id=${body.ragicRecordId}`; if (row.workOrderStatus === 'unknown') row.workOrderStatus = 'idle'; row.workOrderError = null; return row; });
    return { linked };
  }
...
function generatedArtifacts(row: DemoRow, data: DemoDataset): DemoRow {
  ...
  const workOrder = modelRow('StagingWorkOrder', { id: 10000 + transferId, mrpRunId: row.mrpRunId, ragicRecordId: `DEMO-GENERATED-${transferId}`, woNumber, partVersion: row.partVersion, erpPartNo: part.erpPartNo, woQty: row.suggestedQty, subProcessCode: 'HF01', jobOrderCode: '01', startDate: row.completionDate, endDate: row.completionDate, status: '生產中', alreadyPicked: 'No' });
  const templateJob = data.source.work_orders.find((item) => item.partVersion === row.partVersion);
  const bom = data.source.work_order_bom.filter((item) => item.woNumber === templateJob?.woNumber).map((item, index) => { const qty = item.unit === 'kg' ? Number(row.suggestedQty) * Number(part.unitWeightG) / 1000 : Number(row.suggestedQty); return modelRow('StagingWorkOrderBom', { ...item, id: 10000 + transferId * 3 + index, ragicRecordId: `DEMO-GENERATED-BOM-${transferId}-${index}`, woNumber, minUsage: qty, remainingUsage: qty, alreadyPicked: 'No', issuedQty: 0, grossIssuedQty: 0, consumedQty: 0, returnedQty: 0, netIssuedQty: 0, reservedQty: 0, overIssuedQty: 0, issuedDetailCount: 0, movementDetailCount: 0, issuedQtyState: 'not_issued', movementState: 'fallback', issuedQtyError: null, movementError: null, startDate: row.completionDate }); });
