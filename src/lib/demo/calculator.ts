...
export async function calculateSyntheticRun(run: DemoRun, leadTimes: Record<string, number | null> = {}, plans: DemoRow[] = [], progress: (step: string) => Promise<void> = async () => {}, applySkipInventory = false): Promise<{ data: DemoDataset; suggestions: DemoRow[] }> {
  ...
  for (const plan of plans) if (plan.sourceRecordId) data.source.production_plans.push(modelRow('StagingProductionPlan', { id: 5000 + Number(plan.id), mrpRunId: run.id, sourceRecordId: plan.sourceRecordId, planNo: plan.sourcePlanNo, partVersion: plan.partVersion, planQty: plan.suggestedQty, completionDate: plan.completionDate }));
