...
test('Source 連結只導向本機合成紀錄', () => {
  const link = buildSourceRecordUrl('order', 'DEMO-123');
  assert.ok(link?.startsWith('/demo-record?'));
  assert.equal(buildSourceRecordUrl('order', null), null);
});
...
test('合成報表全部具有同 Run/期間/資料識別，庫存分倉守恆', () => {
  for (const id of [1, 2, 3]) {
    ...
    for (const row of data.fg.filter((row) => !row.isAggregated)) {
      ...
      assert.equal(Number(row.mainStockPc) + Number(row.auxStockPc), row.currentStockPc);
...
test('出站 HTTP 的公司網址與亂數 negative control 都在 transport 前被擋', async () => {
  ...
  try {
    ...
    for (const url of ['https://demo.invalid', 'https://not-a-real-company.invalid', 'http://127.0.0.1:8080']) await assert.rejects(fetch(url), /禁止對外/);
