...
export function generateDataset(run: DemoRun = seedRuns()[0]): DemoDataset {
  ...
  for (let i = 1; i <= 24; i++) {
    ...
    const weight = 10 + i / 2;
    const firstProcess = ['成形', '熱處理', '表面處理'][(i - 1) % 3];
    const firstProcessSourceType = ['內製', '委外', '採購'][(i - 1) % 3];
    const firstProcessErpPartNo = `${erp}-P01`;
    ...
    result.source.inventory.push(modelRow('StagingInventory', { id: i, mrpRunId: runId, sourceRecordId: `DEMO-INV-${i}`, erpPartNo: erp, goodStockPc: opening, goodStockKg: opening * weight / 1000, badStockPc: i % 5 * 20, firstProcess, firstProcessErpPartNo, firstProcessSourceType }));
