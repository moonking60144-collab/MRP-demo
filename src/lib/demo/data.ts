...
export function generateDataset(run: DemoRun = seedRuns()[0]): DemoDataset {
  ...
  for (let i = 1; i <= 24; i++) {
    ...
    for (let j = 0; j < 2; j++) {
      ...
      result.sales.push(modelRow('SalesMeeting', { id, mrpRunId: runId, customerCode: customer, customerPartNo, partVersion, memberPartVersions: [partVersion], erpPartNo: erp, unit: 'pc', goodStockPc: opening, goodStockKg: opening * weight / 1000, wfgStockPc: opening - 500, ye1StockPc: 500, badStockPc: i % 5 * 20, badStockKg: i % 5 * 20 * weight / 1000, avgDemandPerWeek: Math.round(orderQty / 4), stockWeeks: opening / (orderQty / 2), shortageStartWeek: salesPeriods.find((period) => Number(period.remainingStock) < 0)?.weekIndex ?? null, purchaseLeadWeeks: 2, outstanding04: recent, fgDiff04: opening - recent * 2, fgStatus04: opening >= recent * 2 ? '足夠' : '不足', totalOrderDemand: orderQty * 12, totalFgDiff: opening - orderQty * 24, sharedErpCount: 2, inventoryValidationAvailable: true, inventoryAnomalyCount: 0, wfgInventoryAnomalyCount: 0, ye1InventoryAnomalyCount: 0, inventoryAnomalyDiffPc: 0, inventoryAnomalyErpPartNos: [] }));
      result.salesPeriods[partVersion] = salesPeriods;
      result.source.part_versions.push(modelRow('StagingPartVersion', { id, mrpRunId: runId, ragicRecordId: `DEMO-PV-${id}`, partVersion, customerCode: customer, customerPartNo, erpPartNo: erp, unitWeightG: weight, forgingParent: row.forgingParent, forgingMachine: row.forgingMachine, firstProcess: '成形', surfaceTreatment: '展示表面處理', processBomVersion: 'DEMO-BOM-V1', productStatus: '使用中' }));
      for (let month = 0; month < 12; month++) {
        result.source.orders.push(modelRow('StagingOrder', { id: id * 100 + month, mrpRunId: runId, ragicRecordId: `DEMO-SO-${id}-${month}`, partVersion, orderQty, shippedQty: 0, unshippedQty: orderQty, deliveryDate: monthStart(month), orderDate: monthStart(month), orderNo: `DEMO-SO-${id}-${month}` }));
        result.source.forecasts.push(modelRow('StagingForecast', { id: id * 100 + month, mrpRunId: runId, ragicRecordId: `DEMO-FC-${id}-${month}`, partVersion, forecastQty, forecastDate: monthStart(month), forecastMonth: monthStart(month) }));
      }
      result.source.production_plans.push(modelRow('StagingProductionPlan', { id, mrpRunId: runId, ragicRecordId: `DEMO-PLAN-${id}`, partVersion, erpPartNo: erp, plannedQty: j === 0 ? plannedQty : 0, completionDate: monthStart(0), planNo: `DEMO-PLAN-${id}` }));
      const woNumber = `DEMO-WO-${id}`;
      result.source.work_orders.push(modelRow('StagingWorkOrder', { id, mrpRunId: runId, ragicRecordId: `DEMO-WO-${id}`, partVersion, erpPartNo: erp, woNumber, jobOrderCode: woNumber, orderQty: j === 0 ? plannedQty : 0, startDate: weekStart(3) }));
      for (const mrpType of ['W', 'B', 'D']) {
        const materialPartNo = `DEMO-${mrpType}-${String((i - 1) % 12 + 1).padStart(3, '0')}`;
        result.source.work_order_bom.push(modelRow('StagingWorkOrderBom', { id: id * 10 + (mrpType === 'W' ? 1 : mrpType === 'B' ? 2 : 3), mrpRunId: runId, ragicRecordId: `DEMO-BOM-${id}-${mrpType}`, componentNo: materialPartNo, woNumber, sourceType: mrpType === 'D' ? '內製' : '外購', processCode: mrpType === 'D' ? '組合' : '成形', unit: mrpType === 'W' ? 'kg' : 'pc', minUsage: j === 0 ? mrpType === 'W' ? plannedQty * weight / 1000 : plannedQty : 0, remainingUsage: j === 0 ? mrpType === 'W' ? plannedQty * weight / 1000 : plannedQty : 0, issuedQtyState: 'not_issued', movementState: 'fallback', startDate: weekStart(3) }));
      }
    }
    result.source.inventory.push(modelRow('StagingInventory', { id: i, mrpRunId: runId, ragicRecordId: `DEMO-INV-${i}`, erpPartNo: erp, goodStockPc: opening, goodStockKg: opening * weight / 1000, badStockPc: i % 5 * 20 }));
    for (const [warehouseCode, quantity] of [['HD', opening - 500], ['YE1', 500]] as const) result.source.inventory_lots.push(modelRow('StagingInventoryLot', { id: i * 2 + (warehouseCode === 'HD' ? 0 : 1), mrpRunId: runId, ragicRecordId: `DEMO-LOT-${i}-${warehouseCode}`, lotNo: `DEMO-LOT-${i}-${warehouseCode}`, erpPartNo: erp, warehouseCode, quantity, stockPc: quantity, unit: 'pc' }));
  }
  for (const mrpType of ['W', 'B', 'D']) for (let i = 1; i <= 12; i++) {
    ...
    result.source.purchase_orders.push(modelRow('StagingPurchaseOrder', { id: i + (mrpType === 'W' ? 0 : mrpType === 'B' ? 100 : 200), mrpRunId: runId, ragicRecordId: `DEMO-PO-${materialPartNo}`, productNo: materialPartNo, orderQty: opening, receiptQty: 0, unreceivedQty: opening, deliveryDate: weekStart(6), unit }));
  }
  ...
  for (const inv of result.source.inventory) Object.assign(inv, { unit: 'pc', subtypeCode: 'FG', inStockPc: inv.goodStockPc, inStockKg: inv.goodStockKg, wfgStockPc: Number(inv.goodStockPc) - 500, ye1StockPc: 500 });
  ...
  for (const row of result.cw) result.source.inventory.push(modelRow('StagingInventory', { id: 1000 + Number(row.id), mrpRunId: runId, ragicRecordId: `DEMO-MATERIAL-${row.materialPartNo}`, erpPartNo: row.materialPartNo, subtypeCode: row.mrpType === 'W' ? 'MTRL-WR' : row.mrpType === 'B' ? 'COMP' : 'SEMI', unit: row.unit, goodStockPc: row.goodStockPc, goodStockKg: row.goodStockKg, purchaseLeadWeeks: 3, purchaseLeadWeeksConfigured: true }));
  ...
  for (const example of cases) {
    ...
    const add = (basisType: string, movementType: string, qty: number, movementQty: number, day: number) => {
      ...
      const row = modelRow('StagingWorkOrderMaterialMovement', { id, mrpRunId: runId, ragicRecordId: `DEMO-MOVEMENT-${id}`, workOrderNo: example.bom!.woNumber, bomItemKey: example.bom!.ragicRecordId, inventoryLotNo: example.lotNo, componentNo: example.bom!.componentNo, movementDate: movementDate.toISOString(), basisType, movementType, inputUnit: 'pc', inputQtyPc: qty, movementQtyPc: movementQty });
  ...
  for (const inv of result.source.inventory.filter((row) => row.subtypeCode !== 'FG')) {
    ...
    for (const index of [1, 2]) result.source.inventory_lots.push(modelRow('StagingInventoryLot', { id: 2000 + result.source.inventory_lots.length, mrpRunId: runId, ragicRecordId: `DEMO-MATERIAL-LOT-${inv.erpPartNo}-${index}`, lotNo: `DEMO-MATERIAL-LOT-${inv.erpPartNo}-${index}`, erpPartNo: inv.erpPartNo, warehouseCode: 'HD', qualityStatus: '正常', stockStatus: '在庫', stockPc: index === 1 ? stockPc - secondPc : secondPc, stockKg: example?.returned ? (index === 1 ? stockPc - secondPc : secondPc) / 100 : stockKg / 2, sourceWorkOrderNo: example?.returned && index === 2 ? example.bom?.woNumber : null }));
    if (example?.bom && !example.returned) result.source.inventory_lots.push(modelRow('StagingInventoryLot', { id: 2000 + result.source.inventory_lots.length, mrpRunId: runId, ragicRecordId: example.lotNo, lotNo: example.lotNo, erpPartNo: inv.erpPartNo, warehouseCode: 'PROCESS', qualityStatus: '正常', stockStatus: example.consumed ? '已結清' : '在製', stockPc: example.gross - example.consumed, stockKg: (example.gross - example.consumed) / 100, sourceWorkOrderNo: example.bom.woNumber }));
  }
  return result;
}
