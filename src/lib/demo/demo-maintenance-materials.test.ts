...
test('元件批號可用量守恆，movement snapshot 原始交易可獨立對回 BOM、週推與來源身分', async () => {
  ...
  for (const [material, type, gross, out, returned, reserved, count] of [
    ...
    const allocations = [{ bomRecordId: String(bom.sourceRecordId), workOrderNo: String(bom.woNumber), bomItemKey: String(bom.sourceRecordId), componentNo: material, inventoryLotNo: detail.movements[0].inventoryLotNo, unit: 'pc', issuedAt: new Date(detail.movements[1].movementDate) }];
    const ledger = resolveWorkOrderMaterialLedgers({ allocations, movements: hydrateRows('StagingWorkOrderMaterialMovement', detail.movementContext) as unknown as WorkOrderLedgerMovement[] }).summaries.get(String(bom.sourceRecordId))!;
    ...
    for (const lot of detail.inventoryLots.filter((row: DemoRow) => row.sourceWorkOrderNo)) assert.equal(lot.sourceWorkOrderSourceRecordId, data.source.work_orders.find(row => row.woNumber === lot.sourceWorkOrderNo)?.sourceRecordId);
    ...
    if (returned) {
      ...
      const feedback = resolveComponentWeeklyMovementInventoryFeedback({ movement, movementIndex: detail.movementContext.findIndex((row: DemoRow) => row.sourceRecordId === movement.sourceRecordId), movements: detail.movementContext, inventoryLots: detail.inventoryLots, unit: 'pc' });
