...
export function usageDetail(data: DemoDataset, materialPartNo: string, params: URLSearchParams): unknown {
  ...
  const movementContext = data.source.work_order_material_movements.filter((row) => row.componentNo === materialPartNo).sort((a, b) => String(a.movementDate ?? '').localeCompare(String(b.movementDate ?? '')) || String(a.ragicRecordId).localeCompare(String(b.ragicRecordId)));
  const movements = movementContext.filter((row) => selectedWorkOrders.has(row.workOrderNo as string));
  const inventoryLots = data.source.inventory_lots.filter((row) => row.erpPartNo === materialPartNo).map((row) => ({ ...row, sourceWorkOrderRagicRecordId: data.source.work_orders.find((job) => job.woNumber === row.sourceWorkOrderNo)?.ragicRecordId ?? null }));
