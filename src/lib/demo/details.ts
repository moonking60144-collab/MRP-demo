...
const link = (row: DemoRow, type: string): DemoRow => ({ ...row, sourceUrl: row.sourceRecordId ? `/demo-record?type=${type}&id=${encodeURIComponent(String(row.sourceRecordId))}` : null });
...
export function warehouseDetail(data: DemoDataset, params: URLSearchParams): unknown {
  ...
  if (!['INTERNAL', 'AUX'].includes(warehouse)) throw new DemoError('倉庫無效。');
  ...
  const lots: DemoRow[] = data.source.inventory_lots.filter((row) => erps.includes(String(row.erpPartNo)) && isAuxWarehouse(String(row.warehouseCode ?? '')) === (warehouse === 'AUX')).map((row) => ({ ...link(row, 'inventory_lot'), includedInMrp: isMrpAvailableInventoryLot({ stockStatus: row.stockStatus, qualityStatus: row.qualityStatus }), sourceWorkOrderSourceUrl: null }));
...
export function usageDetail(data: DemoDataset, materialPartNo: string, params: URLSearchParams): unknown {
  ...
  const selectedWorkOrders = new Set([...result.included, ...result.excluded].map((row) => row.woNumber));
  const movementContext = data.source.work_order_material_movements.filter((row) => row.componentNo === materialPartNo).sort((a, b) => String(a.movementDate ?? '').localeCompare(String(b.movementDate ?? '')) || String(a.sourceRecordId).localeCompare(String(b.sourceRecordId)));
  const movements = movementContext.filter((row) => selectedWorkOrders.has(row.workOrderNo as string));
  const inventoryLots = data.source.inventory_lots.filter((row) => row.erpPartNo === materialPartNo).map((row) => ({ ...row, sourceWorkOrderSourceRecordId: data.source.work_orders.find((job) => job.woNumber === row.sourceWorkOrderNo)?.sourceRecordId ?? null }));
  ...
  return { materialPartNo, mrpType: type, runId: data.run.id, dbSource: params.get('dbSource'), runVersionCode: data.run.versionCode, runDate: data.run.runDate, usesLegacyPurchaseProjection: false, weekIndex: index, weekLabel: index === null ? null : periods.find((row) => row.weekIndex === index)?.weekLabel ?? null, weekStart: index === null ? null : periods.find((row) => row.weekIndex === index)?.weekStart ?? null, inventorySource: { sourceRecordId: inventory.sourceRecordId, erpPartNo: materialPartNo, purchaseLeadWeeks: master ?? 0, purchaseLeadWeeksConfigured: master !== null, ambiguous: false, candidateCount: 1 }, materialSummary: { ...item, initialStock, badStock: Number(type === 'W' ? item.badStockKg : item.badStockPc), supplyQty: periods.filter((row) => index === null || row.weekIndex === index).reduce((sum, row) => sum + Number(row.receipts), 0), plannedUsage: summaryRows.reduce((sum, row) => sum + row.plannedUsage, 0), ...Object.fromEntries((['grossIssuedQty', 'consumedQty', 'returnedQty', 'netIssuedQty', 'reservedQty', 'remainingUsage'] as const).map((key) => [key, sumComplete(key)])), overIssuedQty: summaryRows.reduce((sum, row) => sum + row.overIssuedQty, 0), movementState: resolveComponentWeeklyMovementSummaryState(summaryRows.map((row) => row.movementState)), expeditePurchaseQty: purchaseOrders.filter((row) => row.requiresExpedite).reduce((sum, row) => sum + Number(row.unreceivedQty), 0), expeditePurchaseCount: purchaseOrders.filter((row) => row.requiresExpedite).length }, weeklyProjection: projection, inventoryLots, purchaseOrders, supplyWorkOrders: data.source.work_orders.filter((row) => row.erpPartNo === materialPartNo && isComponentWeeklyDateInScope(row.endDate ? new Date(String(row.endDate)) : null, weeks, index)), movements, movementContext, unscheduledDemand: { count: result.included.filter((row) => row.dateSource === 'missing').length, qty: result.included.filter((row) => row.dateSource === 'missing').reduce((sum, row) => sum + Number(row.remainingUsage), 0) }, ...result };
}
export function updateLeadTime(data: DemoDataset, material: string, params: URLSearchParams, body: DemoRow): unknown {
  ...
  const row = data.source.inventory.find((item) => item.erpPartNo === material && item.sourceRecordId === body.sourceRecordId);
  ...
  return { materialPartNo: material, sourceRecordId: row.sourceRecordId, runId: data.run.id, runVersionCode: data.run.versionCode, previousPurchaseLeadWeeks: previous ?? 0, previousConfigured: configured, purchaseLeadWeeks: value ?? 0, purchaseLeadWeeksConfigured: value !== null, updated: value !== previous, reconciled: false, currentRunUnchanged: true, requiresNewRun: value !== previous };
}
