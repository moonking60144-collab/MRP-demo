...
export function TransferReconciliationControls({
  ...
  const inspectRagic = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(json.error || '無法檢查 Ragic。');
      setInspection(json.inspection as InspectionResult);
    } catch (error) {
      const message = error instanceof Error ? error.message : '無法檢查 Ragic。';
      setInspection(null);
    ...
    try {
      const response = await fetch(
        `/api/fg-monthly/${encodeURIComponent(partVersion)}/suggestions/reconcile`,
        {
          ...
          body: JSON.stringify({
            ...
            planSequence,
            action,
            ragicRecordId: action === 'confirm_created' ? selectedRecordId : undefined,
          }),
        },
      );
      const json = await response.json();
      ...
      showToast({
        ...
        message: action === 'confirm_created'
          ? `已核對並連結 Ragic 生產計畫${result.ragicPlanNo ? ` ${result.ragicPlanNo}` : ''}。`
          : '已標記為未建單，可重新執行轉單。',
        action: result.ragicUrl
          ? { label: '開啟 Ragic', onClick: () => window.open(result.ragicUrl!, '_blank') }
          : undefined,
      });
      try {
        await onResolved();
  ...
  const confirmCreated = async (selectedRecordId: string) => {
    if (!selectedRecordId) {
      showToast({ type: 'error', message: '請先輸入 Ragic Record ID。' });
      return;
    }
  ...
  const confirmNotCreated = async () => {
    const confirmed = await confirmDialog({
      title: '確認 Ragic 未建單',
      message: `請先在 Ragic 確認規劃#${planSequence}確實沒有建立生產計畫。確認後會解除鎖定，允許重新轉單。`,
      confirmText: '確認未建單',
      danger: true,
    });
    ...
    <div className="-mx-3 mb-3 space-y-2.5 border-y border-amber-200 bg-amber-50 px-3 py-3 text-xs">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-amber-950">
          上次建立時與 Ragic 連線中斷。系統已暫停重送，避免產生重複生產計畫。
        </div>
        <button
          type="button"
          onClick={inspectRagic}
        ...
        >
          {checking ? '檢查中...' : inspection ? '重新檢查 Ragic' : '檢查 Ragic 是否已建單'}
        </button>
      </div>
      ...
      {inspection && inspection.candidates.length > 0 && (
        <div className="space-y-2 border border-blue-200 bg-blue-50 px-3 py-2">
          ...
          {inspection.candidates.map((candidate) => (
            <div
              key={candidate.ragicRecordId}
              className="flex flex-wrap items-center justify-between gap-2 border border-blue-100 bg-white px-2.5 py-2"
            >
              <div className="text-slate-700">
                <span className="font-semibold text-slate-900">
                  {candidate.ragicPlanNo || `Record ${candidate.ragicRecordId}`}
                </span>
                <span className="ml-2">#{candidate.ragicRecordId}</span>
              ...
              <button
                type="button"
                onClick={() => confirmCreated(candidate.ragicRecordId)}
                disabled={disabled || isBusy}
      ...
      <details className="text-slate-600">
        <summary className="cursor-pointer select-none text-slate-600 hover:text-slate-900">
          進階：輸入已知的 Ragic Record ID
        </summary>
        <div className="mt-2 flex flex-wrap items-end gap-2">
          <label className="min-w-44 flex-1 sm:max-w-56">
            <span className="mb-1 block">Ragic Record ID</span>
            <input
              type="text"
              inputMode="numeric"
              pattern="[0-9]*"
              ...
              disabled={disabled || isBusy}
              placeholder="例如 5188"
              aria-label={`規劃${planSequence}的 Ragic Record ID`}
