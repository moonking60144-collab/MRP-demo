...
import { buildRagicRecordUrl } from '@/lib/ragic-record-links';
          ...
          <table className="w-full text-xs border-separate border-spacing-0">
            ...
            <tbody>
              {warnings.items.map((item, index) => {
                const href = buildRagicRecordUrl('work-order-bom', item.ragicRecordId);
                return (
                  <tr key={`${item.dbSource || 'local'}-${item.ragicRecordId || index}`} className="odd:bg-white even:bg-amber-50/40">
  ...
  return (
    <div className="overflow-x-auto">
      <table className="text-xs border-collapse">
        ...
        <tbody>
          {/* Remaining Stock row — first to match Ragic [35] */}
          <tr>
