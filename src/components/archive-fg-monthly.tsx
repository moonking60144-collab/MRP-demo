...
export function ArchiveFgMonthly({ run }: { run: ArchiveRunItem }) {
  ...
  const [style, setStyle] = useState<'classic' | 'source'>('classic');
  ...
  const header = useTableColumnHeaderMenu({
    ...
    onFreezeToColumn: id => setFrozen(Math.min(FG_MAX_FROZEN, columns.filter(column => !column.id.startsWith('period.') && !['mainStockPc', 'auxStockPc'].includes(column.id) && colVis.visibility[column.id] !== false).findIndex(column => column.id === id) + 1)),
    canFreezeColumn: id => !id.startsWith('period.') && !['mainStockPc', 'auxStockPc'].includes(id),
  });
  ...
  return <section aria-label="歷史成品月推" className="flex h-full min-h-0 min-w-0 flex-col gap-3 px-4 pt-4 md:px-6 md:pt-6">
    ...
    <div className="flex flex-wrap items-center gap-2 border border-slate-200 bg-white p-3">
      ...
      <label className="text-xs text-slate-600">樣式：<select aria-label="表格樣式" value={style} onChange={event => setStyle(event.target.value as 'classic' | 'source')} className={control}><option value="classic">原本</option><option value="source">Source</option></select></label>
