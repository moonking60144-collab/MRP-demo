...
// 「在 Ragic 開啟」直接跳【業務主表】(ERP 持久資料) 的單筆 detail：
// {base}/default/{path}/{ragicRecordId}。實測 7 個 source 的 staging
// ragicRecordId 都 = 對應業務主表的 record id（通用且穩定，業務主表是 ERP
// 持久資料），點哪筆開哪筆、最直覺。不跳 MRP-[來源]/[MRP專用] 中介表
// （那些是 Ragic MRP 按鈕產生的快照、我們 App 不維護）。
const SOURCE_LINK: Record<SourceKey, { type: RagicRecordType; label: string }> = {
  partVersions:    { type: 'part-version', label: '[10] 客戶料號版本' },
  inventory:       { type: 'inventory', label: '[08] ERP料品資料' },
  orders:          { type: 'order', label: '[02] 訂單明細' },
  forecasts:       { type: 'forecast', label: '[06] 客戶預示量' },
  workOrders:      { type: 'work-order', label: '[92] 工令單' },
  workOrderBoms:   { type: 'work-order-bom', label: '[28] 工令單-BOM' },
  productionPlans: { type: 'production-plan', label: '[10] 生產計畫' },
};

function buildMasterUrl(tabKey: SourceKey, row: Row): string | null {
  return buildRagicRecordUrl(SOURCE_LINK[tabKey].type, String(row.ragicRecordId ?? ''));
}
...
const GENERATION_STATUS: Record<WorkOrderStatus, { label: string; className: string }> = {
  ...
  [WORK_ORDER_STATUS.PENDING]: { label: 'Ragic 執行中', className: 'border-blue-300 bg-blue-50 text-blue-700' },
                      ...
                      {activeRows.map((row, idx) => {
                        ...
                        return (
                          <tr
                            ...
                            title={masterUrl ? `在 Ragic 開啟 ${SOURCE_LINK[activeTab].label}` : undefined}
