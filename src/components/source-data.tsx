...
function sourceRecordTypeForTab(tab: DataTab): RagicRecordType | null {
        ...
        {data.map((row, i) => {
          ...
          return (
            <tr key={i} className={`border-t border-slate-100 hover:bg-slate-50 ${rowClass}`}>
              {visibleCols.map((col) => {
                const isRagicId = col.id === 'ragicRecordId';
                ...
                const recordType = col.id === 'ragicRecordId' ? sourceRecordTypeForTab(tab) : null;
                const workOrderRecordId = col.id === 'sourceWorkOrderNo'
                  ? row.sourceWorkOrderRagicRecordId
                  : row.workOrderRagicRecordId;
                ...
                return (
                  <td
                    ...
                    className={`px-2 py-1 ${
                      ...
                      isRagicId ? 'text-slate-400' : ''
                    } ${isMissingDate ? 'text-red-500 font-medium' : ''}`}
                  >
                    {isMissingDate ? '缺少' : recordType ? (
                      <SourceRecordLink
                        type={recordType}
                        recordId={row.ragicRecordId}
                        label={String(row.ragicRecordId || '來源')}
                      />
                    ) : isWorkOrderLink ? (
                      <SourceRecordLink
                        type="work-order"
                      ...
                      row.anomalyLevel === 'blocking'
                        ...
                        : row.anomalyLevel === 'review'
                          ...
                          : row.anomalyLevel === 'source'
                            ? <span className="rounded bg-blue-100 px-1.5 py-0.5 font-medium text-blue-700">Form 28</span>
