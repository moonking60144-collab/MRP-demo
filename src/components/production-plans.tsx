...
const SEARCHABLE_FIELDS = ['ragicPlanNo', 'partVersion', 'customerCode', 'mrpVersionCode'];
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex min-h-8 flex-wrap items-center gap-x-4 gap-y-1 border-y border-slate-200 bg-slate-50 px-3 py-1.5 text-xs text-slate-600">
        ...
        <span>Ragic 執行中 <strong className="text-blue-700">{workOrderSummary.running}</strong></span>
                ...
                processed.map((item) => (
                  <tr key={item.id}>
                    {colVis.isVisible('ragicPlanNo') && (
                      <td data-col="ragicPlanNo" data-value={item.ragicPlanNo ?? ''} className="font-mono text-xs">
                        {item.ragicUrl ? (
                          <a
                            href={item.ragicUrl}
                            target="_blank"
                          ...
                          >
                            {item.ragicPlanNo || item.ragicRecordId || '—'}
                          </a>
                        ) : (
                          item.ragicPlanNo || '—'
                        )}
