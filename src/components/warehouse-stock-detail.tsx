...
interface WarehouseStockLot {
  ...
  sourceWorkOrderSourceUrl: string | null;
  sourceUrl: string;
}
...
export function WarehouseStockValue({
  ...
  const title = hasInventoryAnomaly
    ? `此數字包含 ${inventoryAnomalyCount} 筆可能未同步的 Source 庫存批號，點擊查看`
...
export function WarehouseStockDrawer({
  ...
  const recheckLot = async (lot: WarehouseStockLot) => {
    setRecheckByRecordId((previous) => ({
      ...previous,
      [lot.sourceRecordId]: { loading: true, error: null, result: previous[lot.sourceRecordId]?.result ?? null },
    }));
    const params = new URLSearchParams({
      runId: String(request.runId),
      recordId: lot.sourceRecordId,
    });
    if (request.dbSource) params.set('dbSource', request.dbSource);
    try {
      ...
      if (!response.ok) throw new Error(body.error || '重新檢查 Source 失敗');
      setRecheckByRecordId((previous) => ({
        ...previous,
        [lot.sourceRecordId]: { loading: false, error: null, result: body },
      }));
    } catch (reason) {
      setRecheckByRecordId((previous) => ({
        ...previous,
        [lot.sourceRecordId]: {
          loading: false,
          error: reason instanceof Error ? reason.message : '重新檢查 Source 失敗',
          result: previous[lot.sourceRecordId]?.result ?? null,
        },
      }));
    }
  };
            ...
            <>
              {!data.inventoryValidationAvailable ? (
                <div className="mb-3 border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                  此歷史 Run 尚無庫存批號數量驗證資料；數字仍採用當時的 Source 快照。
                </div>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-amber-800">
                      MRP 保留 Source 原值計算；請逐筆核對後在 Source 修正，再重跑 MRP。
                    </span>
                  </span>
                        ...
                        >
                          <td className={`px-3 py-2 font-mono ${lot.quantityAnomaly ? 'shadow-[inset_3px_0_0_#f59e0b]' : ''}`}>
                            <a
                              ...
                              title="開啟 Source 庫存批號"
                            >
                              {lot.lotNo || lot.sourceRecordId}
                          ...
                          <td className="px-3 py-2">
                            <div className="flex min-w-[120px] flex-col items-start gap-1">
                              ...
                              {recheckByRecordId[lot.sourceRecordId]?.result?.sourceCorrected && (
                                <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700">
                                  <CheckCircle2 size={11} aria-hidden="true" />Source 已修正，請重跑 MRP
                                </span>
                              )}
                              {recheckByRecordId[lot.sourceRecordId]?.result && !recheckByRecordId[lot.sourceRecordId]?.result?.sourceCorrected && (
                                <span className="text-[10px] font-semibold text-amber-700">Source 目前仍有差異</span>
                              )}
                              {recheckByRecordId[lot.sourceRecordId]?.error && (
                                <span className="text-[10px] text-red-700">{recheckByRecordId[lot.sourceRecordId]?.error}</span>
                              )}
                            </div>
                          </td>
                          <td className="px-3 py-2 text-right font-mono">
                            ...
                            {recheckByRecordId[lot.sourceRecordId]?.result && (
                              <div className="mt-0.5 whitespace-nowrap text-[10px] font-normal text-slate-500">
                                目前 Source {recheckByRecordId[lot.sourceRecordId]?.result?.live.stockPc.toLocaleString()} pc
                              </div>
                            )}
                          </td>
                          ...
                          <td className="px-3 py-2">
                            {lot.sourceWorkOrderNo ? (
                              <div className="min-w-[128px]">
                                {lot.sourceWorkOrderSourceUrl ? (
                                  <a
                                    href={lot.sourceWorkOrderSourceUrl}
                                    target="_blank"
                                    rel="noreferrer"
                            ...
                            <div className="flex items-center justify-center gap-1">
                              {lot.quantityAnomaly && (
                                <button
                                  ...
                                  disabled={recheckByRecordId[lot.sourceRecordId]?.loading}
                                  ...
                                  title="唯讀重新檢查目前 Source 數值"
                                  aria-label={`重新檢查 Source 庫存批號 ${lot.lotNo || lot.sourceRecordId}`}
                                >
                                  <RefreshCw
                                    size={14}
                                    className={recheckByRecordId[lot.sourceRecordId]?.loading ? 'animate-spin' : undefined}
                                  />
                                </button>
                              )}
                              <a
                                ...
                                title="開啟 Source 庫存批號"
                                aria-label={`開啟 Source 庫存批號 ${lot.lotNo || lot.sourceRecordId}`}
                              >
