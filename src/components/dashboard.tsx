...
export function DashboardClient() {
  ...
  // 執行 MRP 的「不計算成品庫存」開關 — 預設不勾(仿照 Ragic,照算真實庫存)
  ...
  const startRun = async () => {
    ...
    try {
      while (true) {
        ...
        if (!res.ok) {
          setRunError(formatRagicPreflightError(json, '執行失敗'));
          setStarting(false);
          setUpdatingNotice(false);
          return;
        }
    ...
    <div className="h-full overflow-auto" data-dashboard-scroll>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3 justify-end">
          <SystemStatusDrawer onRefresh={() => void fetchMaintenance()} loading={maintenanceLoading} incomplete={Boolean(maintenanceError || !maintenance?.ragic)}
          ...
          <label
            ...
            title="勾選了「不計算成品庫存」設定的料件,執行時把成品庫存當 0 算;不勾則照算真實庫存（同 Ragic）"
          >
            <input
              type="checkbox"
              checked={applySkipFg}
...
// divergent record + Ragic 直達連結 + 中文操作指引。
// ============================================================
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="bg-amber-50 border-b border-amber-200 px-5 py-4">
          <h3 className="text-lg font-bold text-amber-900">⚠ Ragic 公式未自動重算</h3>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-3 text-sm">
            ...
            {divs.map((d, i) => (
              <div key={`${d.recordId}-${i}`} className="border border-slate-200 rounded-lg p-3 bg-slate-50">
                ...
                {d.reason === 'fetch_failed' ? (
                  <div className="text-xs text-red-600 mt-1">Ragic 端撈取失敗 (record id: {d.recordId})</div>
                ) : (
                  <div className="text-xs font-mono mt-1">
                    Ragic 值：<b>{Number(d.ragic).toLocaleString()}</b> ／ 我們算的：
                ...
                {d.ragicUrl && (
                  <div className="mt-2 flex items-center gap-3">
                    ...
                    <button
                      onClick={() => navigator.clipboard.writeText(d.ragicUrl!)}
          ...
          <div className="border-t border-slate-200 pt-3 text-xs text-slate-600 space-y-1">
            <div>通常是 Ragic 子表單欄位 (AC11) 有變動但 G5 欄位沒被觸發重算造成的。</div>
            <div>
...
function MaintenanceOverview({ data }: { data: MaintenanceData }) {
  ...
  const ragicStatusMap: Record<NonNullable<MaintenanceData['ragic']>['status'], { label: string; color: string }> = {
  ...
  const ragicStatus = data.ragic
    ? ragicStatusMap[data.ragic.status]
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white" aria-label="系統狀態">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Ragic 唯讀連線探測、自動備份與 Run 保留狀態；此處不執行資料清理。
          </p>
        </div>
      ...
      <div className="space-y-3 p-4 text-xs">
        {[{ name: 'Ragic 連線', status: ragicStatus, at: data.ragic?.lastSuccessAt },
      ...
      <div className="grid grid-cols-1 divide-y divide-slate-200">
        <div className="p-4">
          <div className="flex items-center justify-between gap-3">
            <h4 className="text-sm font-medium text-slate-700">Ragic API 連線</h4>
            <span className={`rounded px-2 py-0.5 text-[11px] font-medium ${ragicStatus.color}`}>
              {ragicStatus.label}
            </span>
          </div>
          {data.ragic ? (
          <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
            ...
            <dd className="text-right font-medium text-slate-700">
              {new Date(data.ragic.checkedAt).toLocaleString('zh-TW')}
            </dd>
            <dt className="text-slate-500">最近成功</dt>
            <dd className="text-right font-medium text-slate-700">
              {data.ragic.lastSuccessAt
                ? new Date(data.ragic.lastSuccessAt).toLocaleString('zh-TW')
                : '尚無'}
            </dd>
            <dt className="text-slate-500">探測總耗時</dt>
            <dd className="text-right font-medium text-slate-700">
              {formatDurationMs(data.ragic.timings.totalMs)}
            </dd>
            <dt className="text-slate-500">DNS／TCP／TLS</dt>
            <dd className="text-right font-medium text-slate-700">
              {formatDurationMs(data.ragic.timings.dnsMs)}／
              {formatDurationMs(data.ragic.timings.tcpMs)}／
              {formatDurationMs(data.ragic.timings.tlsMs)}
            </dd>
            <dt className="text-slate-500">等待回應 TTFB</dt>
            <dd className="text-right font-medium text-slate-700">
              {formatDurationMs(data.ragic.timings.ttfbMs)}
            </dd>
            {data.ragic.error && (
              <>
                <dt className="text-red-600">錯誤</dt>
                <dd className="break-words text-right font-medium text-red-700" title={data.ragic.error}>
                  {data.ragic.error}
                </dd>
              </>
            )}
            ...
            <p className="mt-3 text-xs leading-5 text-slate-500">
              尚無持久化探測紀錄；後端已在背景檢查 Ragic，完成後會自動更新。
            </p>
          )}
        </div>
