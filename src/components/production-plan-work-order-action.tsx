...
interface ProductionPlanWorkOrderActionProps {
  ...
  ragicRecordId: string | null | undefined;
  ragicPlanNo?: string | null;
  ragicUrl?: string | null;
...
export function ProductionPlanWorkOrderAction({
  ...
  const [linkedRecordId, setLinkedRecordId] = useState<string | null>(ragicRecordId ?? null);
  const [linkedPlanNo, setLinkedPlanNo] = useState<string | null>(ragicPlanNo ?? null);
  const [linkedUrl, setLinkedUrl] = useState<string | null>(ragicUrl ?? null);
  ...
  useEffect(() => {
    ...
    setLinkedRecordId(ragicRecordId ?? null);
    setLinkedPlanNo(ragicPlanNo ?? null);
    setLinkedUrl(ragicUrl ?? null);
  }, [initialStatus, initialError, ragicPlanNo, ragicRecordId, ragicUrl, transferId]);
  ...
  const linkRecord = async () => {
    ...
    try {
      const response = await fetch(`/api/production-plans/${transferId}/record-link`, {
        ...
        body: JSON.stringify({ ragicRecordId: recordIdInput }),
      });
      ...
      setLinkedRecordId(linked.ragicRecordId);
      setLinkedPlanNo(linked.ragicPlanNo);
      setLinkedUrl(linked.ragicUrl);
      ...
      onChange?.({
        ...
        ragicRecordId: linked.ragicRecordId,
        ragicPlanNo: linked.ragicPlanNo,
        ragicUrl: linked.ragicUrl,
      });
      showToast({
        type: 'success',
        message: `已驗證並連結生產計畫${linked.ragicPlanNo ? ` ${linked.ragicPlanNo}` : ''}。`,
        action: { label: '開啟生產計畫', onClick: () => window.open(linked.ragicUrl, '_blank') },
      });
    } catch (linkError) {
      showToast({
        type: 'error',
  ...
  const run = async () => {
    ...
    if (status !== WORK_ORDER_STATUS.UNKNOWN) {
      const confirmed = await confirmDialog({
        title: '產生工令單',
        message: `將把 Ragic 生產計畫${linkedPlanNo ? ` ${linkedPlanNo}` : ''}加入背景佇列，依序執行「載入製程並推估時間」。\n\n此動作會展開製程、計算時程並產生工令單。送出後可離開此頁，系統會持續執行並更新進度。`,
        confirmText: '加入背景佇列',
      });
      if (!confirmed) return;
    }
    ...
    try {
      ...
      showToast({
        type: 'success',
        message: result.alreadyGenerated ? 'Ragic 已有工令單，狀態已同步。' : '工令單產生完成。',
        action: linkedUrl
          ? { label: '開啟生產計畫', onClick: () => window.open(linkedUrl, '_blank') }
          : undefined,
      });
    ...
    if (!linkOpen) {
      return (
        <button
          ...
          title="生產計畫已建立，但 Ragic 成功回應未附 Record ID；可唯讀驗證後補上連結"
    ...
    return (
      <div className="inline-flex items-center gap-1 whitespace-nowrap">
        <input
          ...
          aria-label="Ragic 生產計畫 Record ID"
  ...
  return (
    <button
      ...
      title={error || (isUnknown
        ? '只會重新讀取 Ragic 狀態，不會在結果不明時重跑 Button 92'
        : isQueued
          ? '已加入背景佇列，尚未輪到此筆'
          : isRunning
            ? 'Ragic 正在展開製程、推估時間並產生工令'
            : '加入背景佇列，在 Ragic 執行「載入製程並推估時間」')}
