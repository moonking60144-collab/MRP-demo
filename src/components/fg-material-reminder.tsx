...
export function FgMaterialReminderPanel({ item, archiveId, initiallyOpen = false }: { item: Target; archiveId?: string; initiallyOpen?: boolean }) {
  ...
  return <section aria-label="關聯材料提醒" className="my-3 overflow-hidden rounded border border-slate-200 bg-white text-xs text-slate-700">
    ...
    {!!reminder?.rows.length && <div className="mt-3 overflow-auto"><table className="w-full whitespace-nowrap text-left"><thead className="bg-slate-100"><tr>{['客料版本', '來源工令', '關聯材料', '需求日期', '尚需用量', '材料首次缺料', '查詢'].map(label => <th key={label} className="p-2 font-medium">{label}</th>)}</tr></thead>
      <tbody>{reminder.rows.map((row, index) => <tr key={index} className="border-b border-slate-100"><td className="p-2">{row.partVersion}</td><td className="p-2">{row.woNumber ?? '—'}</td><td className="p-2 font-mono">{row.materialPartNo ?? '—'}</td><td className="p-2">{row.demandDate?.slice(0, 10) ?? '待確認'}</td><td className="p-2">{row.issuedQtyState === 'unknown' || row.remainingUsage === null ? '待確認' : `${Number(row.remainingUsage).toLocaleString()} ${row.unit ?? ''}`}</td><td className="p-2">{!row.mrpType ? '未取得週推資料' : row.shortageStartWeek === null ? '未見缺料' : row.shortageStartWeek === 0 ? '前期未結' : `W${row.shortageStartWeek}${row.shortageStartDate ? ` · ${row.shortageStartDate.slice(0, 10)}` : ''}`}</td><td className="p-2">{row.materialPartNo && row.mrpType && <Link className="text-blue-700 underline" href={materialWeeklyHref(scope, row.materialPartNo, row.mrpType)} target="_blank" rel="noopener noreferrer" title="在新分頁查看元件週推">查看元件週推<span className="sr-only">（新分頁）</span></Link>}</td></tr>)}</tbody></table></div>}
      </div>
    </details>
  </section>;
}
