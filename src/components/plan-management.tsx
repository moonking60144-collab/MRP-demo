...
const SEARCHABLE_FIELDS = ['partVersion', 'customerPartNo', 'customerCode', 'erpPartNo', 'forgingParent', 'ragicPlanNo'];
...
// ragic-client 無 global rate limiter，唯一護欄是 per-call 的 429 backoff；
...
export function PlanManagementClient() {
  ...
  const handleBatchTransfer = async () => {
    ...
    const ok = await confirmDialog({
      title: '批次建立生產計畫',
      message: `確定要在 Ragic 建立 ${selectedItems.length} 筆生產計畫？${anomalyMessage}\n\n此步驟不會產生工令單；建立完成後再由使用者逐筆確認並執行。`,
      confirmText: '建立生產計畫',
    });
    if (!ok) return;
                ...
                pagedProcessed.map((item) => {
                  ...
                  return (
                    <tr key={item.id} onClick={(e) => handleRowClick(item, e)}
                      ...
                      {colVis.isVisible('ragicPlanNo') && (
                        <td className="font-mono text-xs">
                          {item.ragicUrl ? (
                            <a href={item.ragicUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">{item.ragicPlanNo || '—'}</a>
                          ) : item.ragicPlanNo || '—'}
                        </td>
                      )}
                      <td>
                        <div className="flex items-center gap-1">
                          ...
                          {item.status === '已轉單' && (
                            <>
                              {item.ragicUrl && (
                                <a href={item.ragicUrl} target="_blank" rel="noopener noreferrer"
                              ...
                              <ProductionPlanWorkOrderAction
                                ...
                                ragicRecordId={item.ragicRecordId}
                                ragicPlanNo={item.ragicPlanNo}
                                ragicUrl={item.ragicUrl}
...
function PlanDetailOverlay({
  ...
  const handleTransfer = async (seq: number) => {
    ...
    const inventoryWarning = item.inventoryAnomalyCount > 0
      ? `\n\n注意：此料號有 ${item.inventoryAnomalyCount} 筆庫存批號可能尚未同步，差異絕對值合計 ${item.inventoryAnomalyDiffPc.toLocaleString()} pc。MRP 仍採 Ragic 原值。`
      : '';
    const ok = await confirmDialog({
