...
interface UsageDetailRow {
  ...
  workOrderRagicRecordId: string | null;
...
interface InventoryLotRow {
  ...
  sourceWorkOrderRagicRecordId: string | null;
}
...
function SourceLink({
  ...
  const href = buildRagicRecordUrl(type, recordId);
...
export function ComponentWeeklyUsageDrawer({
  ...
  const saveLeadTime = async (clearValue: boolean) => {
    if (!data?.inventorySource?.ragicRecordId || leadTimeSaving) return;
    ...
    try {
      ...
      const response = await fetch(
        `/api/component-weekly/${encodeURIComponent(data.materialPartNo)}/purchase-lead-time?${params}`,
        {
          ...
          body: JSON.stringify({
            ragicRecordId: data.inventorySource.ragicRecordId,
      ...
      setLeadTimeSuccess(
        result.updated
          ? `Ragic 已更新為「${currentLabel}」；本 Run 仍保留原快照，建立下一個 Run 後才會套用。`
          : `Ragic 已是「${currentLabel}」，沒有重複寫入；本 Run 不需變更。`,
      );
      setLeadTimeUpdateCompleted(Boolean(result.updated));
      setLeadTimeEditorOpen(false);
    } catch (saveError) {
      setLeadTimeError(saveError instanceof TypeError
        ? '連線中斷，前置期更新結果無法確認；請先開啟 Ragic 原單核對，確認前不要重送。'
            ...
            <div className="min-h-0 shrink overflow-y-auto overscroll-contain">
              ...
              {data.unscheduledDemand.count > 0 && (
                <section className="flex items-center gap-1.5 border-b border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-900 sm:px-5">
                  <AlertTriangle size={15} />
                  <span>
                    此範圍包含 {data.unscheduledDemand.count} 筆未排程需求，共 {formatNumber(data.unscheduledDemand.qty)} {unit}；Form 96 與 Form 28 都沒有日期，因此保守列入前期。
                  </span>
                </section>
              )}
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-semibold">
                  {data.materialSummary.movementState === 'known'
                    ? '領用、帳面出庫、正式退料與工令保留已依本 Run 的 Form 20 批號快照計算；歷史 Run 不會因工令後續結案而回寫，請以正式退料與最新 Run 判讀'
                    : data.materialSummary.movementState === 'unknown'
                      ? '部分耗用／退料無法唯一歸屬，該工令不納入需求'
                      : data.materialSummary.movementState === 'mixed'
                        ? '部分工令依 Form 20 計算，其餘沿用 Form 28 領料結果'
                      : '此 Run 尚無 Form 20 快照，沿用 Form 28 領料結果'}
                </span>
              </div>
                  ...
                  <section className="rounded border border-slate-200 bg-white p-4">
                    ...
                    <div className={`mt-3 border-t pt-3 text-xs ${
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          ...
                          {data.inventorySource?.ambiguous && (
                            <div className="mt-1 text-[11px] font-medium">
                              已停用主檔連結與前置期寫回；請先整理 Ragic 的重複 ERP 料號。
                            </div>
                          )}
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-2">
                          {data.inventorySource?.ragicRecordId ? (
                            <SourceLink
                              type="inventory-master"
                              recordId={data.inventorySource.ragicRecordId}
                              label="開啟 Ragic 料號主檔"
                            />
                          ) : (
                            <span className="font-medium text-slate-400">無可用主檔連結</span>
                          )}
                          {!readOnly && data.mrpType !== 'D' && data.inventorySource?.ragicRecordId && (
                            <button
                              type="button"
                      ...
                      {leadTimeEditorOpen && data.inventorySource && (
                        <div className="mt-3 rounded border border-blue-200 bg-blue-50/70 p-3 text-slate-700">
                          ...
                          <p className="mt-2 text-[11px] leading-5 text-slate-500">
                            只更新 Ragic 料號主檔欄位；本 Run 是不可變快照，仍顯示目前數值，下一個 MRP Run 才會採用新設定。
                          </p>
                        </div>
                      )}
                    ...
                    <p className="mt-3 border-t border-slate-200 pt-3 text-xs leading-5 text-slate-500">
                      ...
                      {data.usesLegacyPurchaseProjection
                        ...
                        : ' 逾期未交 PO 不當成已到貨，請至「採購未交」核對並開啟 Ragic 原單。'}
                    </p>
                  </section>
                </div>
              )}
                ...
                <>
                  ...
                  {rows.length === 0 ? (
                    ...
                    <table className="w-full min-w-[1460px] border-separate border-spacing-0 text-xs">
                      ...
                      <tbody>
                        {rows.map((row, index) => (
                          <tr key={`${row.ragicRecordId || row.woNumber || 'row'}-${index}`} className="odd:bg-white even:bg-slate-50/70">
                            <td className="border-b border-slate-100 px-3 py-2 font-mono font-medium">
                              <SourceLink type="work-order" recordId={row.workOrderRagicRecordId} label={row.woNumber || '工令'} />
                            </td>
                            ...
                            <td className="border-b border-slate-100 px-3 py-2">
                              <div className="flex flex-wrap items-center gap-1.5" title={row.dateMismatch
                                ? `Form 96：${row.scheduleStartDate || '—'}；Form 28：${row.bomStartDate || '—'}`
                                : undefined}
                              >
                            ...
                            <td className="border-b border-slate-100 px-3 py-2">
                              <div className="flex flex-wrap items-center gap-1.5">
                                <SourceLink type="work-order-bom" recordId={row.ragicRecordId} label="BOM" />
                  ...
                  <table className="w-full min-w-[860px] border-separate border-spacing-0 text-xs">
                    ...
                    <tbody>
                      {data.inventoryLots.map((row) => (
                        <tr key={row.ragicRecordId} className="odd:bg-white even:bg-slate-50/70">
                          ...
                          <td className="border-b border-slate-100 px-3 py-2 font-mono">
                            <SourceLink
                              type="work-order"
                              recordId={row.sourceWorkOrderRagicRecordId}
                              label={row.sourceWorkOrderNo || '—'}
                            />
                          </td>
                          <td className="border-b border-slate-100 px-3 py-2">
                            <SourceLink type="inventory-lot" recordId={row.ragicRecordId} label="批號" />
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <table className="w-full min-w-[920px] border-separate border-spacing-0 text-xs">
                    ...
                    <tbody>
                      {data.purchaseOrders.map((row, index) => (
                        <tr
                          key={`${row.ragicRecordId || 'po'}-${index}`}
                        ...
                        >
                          ...
                          <td className="border-b border-slate-100 px-3 py-2">
                            <SourceLink type="purchase-order" recordId={row.ragicRecordId} label="採購" />
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <table className="w-full min-w-[760px] border-separate border-spacing-0 text-xs">
                    ...
                    <tbody>
                      {data.supplyWorkOrders.map((row, index) => (
                        <tr key={`${row.ragicRecordId || 'wo'}-${index}`} className="odd:bg-white even:bg-slate-50/70">
                          ...
                          <td className="border-b border-slate-100 px-3 py-2">{row.status || '—'}</td>
                          <td className="border-b border-slate-100 px-3 py-2">
                            <SourceLink type="work-order" recordId={row.ragicRecordId} label="工令" />
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              {tab === 'movements' && (
                data.movements.length === 0 ? (
                  <div className="flex h-48 items-center justify-center px-6 text-center text-sm text-slate-400">
                    此範圍沒有 Form 20 耗用／退料快照；若上方顯示 fallback，代表此 Run 沿用 Form 28 領料資料
                  </div>
                ) : (
                    ...
                    <tbody>
                      {data.movements.map((row) => {
                        ...
                        const movementContextIndex = data.movementContext.findIndex(
                          (candidate) => candidate.ragicRecordId === row.ragicRecordId,
                        );
                        ...
                        return (
                          <tr key={row.ragicRecordId} className="odd:bg-white even:bg-slate-50/70">
                                ...
                                {inventoryFeedback.state === 'in_stock' && (
                                  <>
                                    ...
                                    {inventoryFeedback.inventoryLots.map((inventoryLot) => (
                                      <SourceLink
                                        key={inventoryLot.ragicRecordId}
                                        type="inventory-lot"
                                        recordId={inventoryLot.ragicRecordId}
                            ...
                            <td className="border-b border-slate-100 px-3 py-2">
                              <SourceLink type="inventory-movement" recordId={row.ragicRecordId} label="異動" />
                            </td>
                          </tr>
                        );
                      })}
