...
export function TraditionalView({
  ...
  showTree?: boolean;
  /** 表格樣式：'classic' = 原本、'source' = 比照 Source 條件式格式。由父 component 控制
      （toolbar 切換器與「視圖」並排），純前端 UI，不影響業務數值。 */
  tableStyle?: 'classic' | 'source';
  ...
  /** Opens the current Run's internal/AUX lot snapshot without reading live Source. */
  onShowWarehouseStock?: (
    item: FgMonthlyItem,
  ...
  const selectionSummary = useMemo(() => {
    ...
    const sharedPrePeriodKeys = new Set([
      'mainStockPc', 'auxStockPc', 'currentStockPc', 'badStockPc', 'lastPeriodRemainingNoPlan',
    ]);
  ...
  const renderRow = useCallback((item: FgMonthlyItem, rowIdx: number) => {
            ...
            // 否則回退逐列白/灰斑馬。
            // Source 模式下強制關掉「同鍛造母件淡藍」群組視覺 — Source 端原本就沒這效果，
            // 且 Source 自己有整列染色 + 斑馬紋會打架，視覺對齊不出來。
            const groupZebra = tableStyle !== 'source' && !showTree && groupLines.byForging;
            ...
            } else if (tableStyle !== 'source' && !showTree && groupLines.customerStart[rowIdx]) {
              // 同一鍛造母件群組內客戶料號換值 → 色帶內畫一條粗分隔線（底色不變，仍是同母件）
              // Source 模式下關掉 — Source 端原本沒這線、且 row 級染色已能視覺區分
              ...
              >
                ...
                {visiblePrePeriodCols.map((col, gi) => {
                  ...
                  if (col.group === 'status') {
                    ...
                    // 開始缺貨期數 cell 按值 3 階分級（對齊 Source 色碼）：
                  ...
                  // data-* 屬性給 Source 模式 CSS rule 用：data-col 識別欄位、
                ...
                {timelineSections.map((section, sectionIdx) => {
                            ...
                            >
                              {snapshot ? <SharedBalanceValue value={value} label={column.label} sharedErpCount={item.sharedErpCount} erpPartNo={item.erpPartNo} /> : <WarehouseStockValue
                                ...
                                hasInventoryAnomaly={warehouseGroup === 'INTERNAL'
                                  ? item.mainInventoryAnomalyCount > 0
                                  : item.auxInventoryAnomalyCount > 0}
                                inventoryAnomalyCount={warehouseGroup === 'INTERNAL'
                                  ? item.mainInventoryAnomalyCount
                                  : item.auxInventoryAnomalyCount}
                  ...
                  {paddedPeriods.map((p, mi) => {
                    ...
                    // data-period-neg: 剩餘庫存（含無計劃量）各期 < 0 給 Source 模式
                    // 用 — CSS rule 覆蓋 row 級的 critical/warning 染色（避免警示被
                    // 「整列同色」吃掉，對齊 Source 條件清單獨立列出的 ST1-10~21 / S01~S09）
                                    ...
                                    : member.inventoryAnomalyCount > 0
                                      ? <button
                                          ...
                                          onClick={(event) => {
                                            ...
                                            onShowWarehouseStock?.(
                                              member,
                                              member.mainInventoryAnomalyCount > 0 ? 'INTERNAL' : 'AUX',
                                              'ANOMALY',
                                            );
                                          }}
                                          className="inline-flex items-center gap-1 border border-amber-500 bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold text-amber-950 hover:bg-amber-200"
                          ...
                          if (col.key === 'currentStockPc' || col.key === 'badStockPc') formatted = (
                            <SharedBalanceValue
                              ...
                              warning={col.key === 'currentStockPc' && member.inventoryAnomalyCount > 0
                                ? `${member.inventoryAnomalyCount} 筆庫存批號可能尚未同步；MRP 仍採 Source 原值`
                                : undefined}
                            />
                          );
                                  ...
                                  return (
                                    ...
                                    >
                                      <WarehouseStockValue
                                        ...
                                        warehouseGroup={warehouseGroup}
                                        hasInventoryAnomaly={warehouseGroup === 'INTERNAL'
                                          ? member.mainInventoryAnomalyCount > 0
                                          : member.auxInventoryAnomalyCount > 0}
                                        inventoryAnomalyCount={warehouseGroup === 'INTERNAL'
                                          ? member.mainInventoryAnomalyCount
                                          : member.auxInventoryAnomalyCount}
  ...
  return (
    ...
    <div className="flex-1 min-h-0 flex flex-col">
      ...
      <div className="flex-shrink-0 flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-t-lg text-[10px] flex-wrap">
        ...
        <SharedErpLegend />
        {tableStyle !== 'source' && !showTree && groupLines.byForging && (
      ...
      >
        ...
        <thead className="sticky top-0 z-30">
          ...
          <tr>
            ...
            {/* Period year-month — top row (對齊 Source d4/22 版面)。每格加上
