...
interface FgMonthlyItem {
  ...
  mainInventoryAnomalyCount: number;
  auxInventoryAnomalyCount: number;
...
export function FgMonthlyClient() {
  ...
  // 傳統 view 樣式切換：'classic' = 既有版面；'source' = 比照 Source 條件式格式
  // （斑馬紋 + 不良品庫存 > 0 紅 + 開始缺貨期數 cell 按值分級染色）。純前端 UI，
  // 不影響業務數值。lift up 到這層是為了讓 toolbar 切換器放主視圖區（與「視圖」並排）。
  const [tableStyle, setTableStyle] = usePersistedState<'classic' | 'source'>('mrp_trad_style', 'classic');
      ...
      <div className="flex items-center gap-1 text-xs">
        ...
        {viewMode === 'traditional' && (
          <label className="flex items-center gap-1 text-slate-600">
            ...
            <select
              value={tableStyle}
              onChange={(e) => setTableStyle(e.target.value as 'classic' | 'source')}
              className="px-1.5 py-1 border border-slate-300 rounded text-xs"
              title="切換表格樣式（純前端 UI，不影響業務數值）"
            >
              <option value="classic">原本</option>
              <option value="source">Source 風</option>
            </select>
          </label>
        )}
        {viewMode === 'traditional' && (
          ...
          <KeyboardScrollHelp
            ...
            extraSection={viewMode === 'traditional' ? (
              <>
                {tableStyle === 'source' ? (
                  <>
                    <div className="font-semibold text-slate-700 mb-1">Source 風樣式條件</div>
        ...
        <table className="w-full mrp-table" style={{ zoom: TEXT_ZOOM_LEVELS[textSize] || 1 }}>
          <thead>
            <tr>
              ...
              {colVis.isVisible('mainStockPc') && <ColumnHeaderCell column={columns.find((column) => column.id === 'mainStockPc')!} controller={columnHeader.menuController} className="bg-sky-50" />}
              {colVis.isVisible('auxStockPc') && <ColumnHeaderCell column={columns.find((column) => column.id === 'auxStockPc')!} controller={columnHeader.menuController} className="bg-sky-50" />}
                      ...
                      <td className="text-right font-mono text-xs">
                        <SharedBalanceValue
                          ...
                          warning={item.inventoryAnomalyCount > 0
                            ? `${item.inventoryAnomalyCount} 筆庫存批號可能尚未同步；MRP 仍採 Source 原值`
                            : undefined}
                        />
                      </td>
                    )}
                    ...
                    {colVis.isVisible('auxStockPc') && (
                      <td className="bg-sky-50 text-right font-mono text-xs">
                        <WarehouseStockValue
                          ...
                          onOpen={() => openWarehouseStock(item, 'AUX')}
                        />
                      </td>
                    )}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-slate-500 mt-1">
                    ...
                    <b className="text-slate-700 font-mono">
                      <SharedBalanceValue
                        ...
                        warning={expandedItem.inventoryAnomalyCount > 0
                          ? `${expandedItem.inventoryAnomalyCount} 筆庫存批號可能尚未同步；MRP 仍採 Source 原值`
                          : undefined}
                      />
                    </b>
                  </span>
                  <span className="inline-flex items-center gap-1">
                    ...
                    <WarehouseStockValue
                      value={expandedItem.mainStockPc}
                      warehouseGroup="INTERNAL"
                      hasInventoryAnomaly={expandedItem.mainInventoryAnomalyCount > 0}
                      inventoryAnomalyCount={expandedItem.mainInventoryAnomalyCount}
                      onOpen={() => openWarehouseStock(expandedItem, 'INTERNAL')}
                    />
                  </span>
                  <span className="inline-flex items-center gap-1">
                    {WAREHOUSE_STOCK_GROUPS.AUX.label}:
                    <WarehouseStockValue
                      value={expandedItem.auxStockPc}
                      warehouseGroup="AUX"
                      hasInventoryAnomaly={expandedItem.auxInventoryAnomalyCount > 0}
                      inventoryAnomalyCount={expandedItem.auxInventoryAnomalyCount}
                      onOpen={() => openWarehouseStock(expandedItem, 'AUX')}
                    />
                  </span>
                ...
                {expandedItem.inventoryAnomalyCount > 0 && (
                  ...
                  >
                    ...
                    <span className="text-amber-800">
                      差異絕對值合計 {expandedItem.inventoryAnomalyDiffPc.toLocaleString()} pc，MRP 仍採 Source 原值。
                    </span>
                    <span className="flex-1" />
                    {expandedItem.mainInventoryAnomalyCount > 0 && (
                      <button
                        type="button"
                      ...
                      >
                        查看廠內異常 {expandedItem.mainInventoryAnomalyCount}
                        <ChevronRight size={13} aria-hidden="true" />
                      </button>
                    )}
                    {expandedItem.auxInventoryAnomalyCount > 0 && (
                      <button
                        type="button"
                        onClick={() => openWarehouseStock(expandedItem, 'AUX', 'ANOMALY')}
                        className="inline-flex items-center gap-1 border border-amber-500 bg-white px-2 py-1 font-semibold text-amber-900 hover:bg-amber-100"
                      >
                        查看 AUX 異常 {expandedItem.auxInventoryAnomalyCount}
                        <ChevronRight size={13} aria-hidden="true" />
                      </button>
                    )}
                  </div>
...
function PeriodDetailViewDetailed({
  ...
  const handleTransfer = async (seq: number) => {
    ...
    const inventoryWarning = inventoryAnomalyCount > 0
      ? `\n\n注意：此料號有 ${inventoryAnomalyCount} 筆庫存批號可能尚未同步，差異絕對值合計 ${inventoryAnomalyDiffPc.toLocaleString()} pc。MRP 仍採 Source 原值。`
      : '';
    ...
    try {
      ...
      if (res.ok) {
        ...
        const recordTag = t?.sourceRecordId ? `（Record ${t.sourceRecordId}）` : '';
        showToast({
          type: 'success',
...
// ============================================================
const FG_SOURCE_SOURCE_RECORD_TYPE: Record<FgMonthlySourceType, SourceRecordType> = {
  orders: 'order',
  forecasts: 'forecast',
...
function SourceRecordLink({
  ...
  const href = buildSourceRecordUrl(
    FG_SOURCE_SOURCE_RECORD_TYPE[type],
      ...
      <table className="w-full min-w-[1840px] table-fixed border-separate border-spacing-0 bg-white text-xs">
        <thead className="bg-slate-50">
          <tr className="text-slate-600">
            <th rowSpan={2} className="w-24 border-b border-slate-200 px-3 py-2 text-center font-semibold">Source ID</th>
        ...
        <tbody>
          {records.map((r: Record<string, unknown>, i: number) => {
            ...
            return (
              <tr key={String(r.sourceRecordId ?? i)} className={manualClose ? 'bg-amber-50/40 hover:bg-amber-50' : 'hover:bg-blue-50/50'}>
                <td className="border-b border-slate-100 px-3 py-2 text-center"><SourceRecordLink type={type} recordId={r.sourceRecordId} /></td>
  ...
  if (type === 'forecasts') {
    return (
      <table className="mx-auto w-full max-w-[760px] table-fixed border-separate border-spacing-0 bg-white text-xs">
        <thead className="bg-slate-50">
          <tr className="text-slate-600">
            <th className="w-28 border-b border-slate-200 px-3 py-2 text-center font-semibold">Source ID</th>
  ...
  if (type === 'work_orders') {
    return (
      <table className="mx-auto w-full min-w-[980px] table-fixed border-separate border-spacing-0 bg-white text-xs">
        <thead className="bg-slate-50">
          <tr className="text-slate-600">
            <th className="w-24 border-b border-slate-200 px-3 py-2 text-center font-semibold">Source ID</th>
