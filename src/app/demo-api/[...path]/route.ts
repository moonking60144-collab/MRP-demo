...
export async function GET(request: NextRequest, context: Context) {
  try {
    ...
    if (key === 'settings') return json({ dbMode: state().dbMode, urls: { local: '合成資料 A（無連線）', docker: '合成資料 B（無連線）', remote: '合成資料 C（無連線）' }, urlConfigured: { local: true, docker: true, remote: true }, connection: { connected: true, dbVersion: 'Interview demo・不連資料庫', error: '' } });
    if (key === 'maintenance-status') return json({ source: null, backup: { enabled: false, ready: false, due: false, activeRun: false, intervalHours: 24, retentionDays: 30, minimumBackups: 3, lastAutomaticResult: null, latestBackup: null }, retention: { enabled: false, activeRun: false, retentionDays: 30, minimumCompletedRuns: 30, batchSize: 10, completedRunCount: runs().length, eligibleRunCount: 0, blockedRunCount: 0, nextEligibleRunId: null, nextCoverageReadyRunId: null, archiveGate: { enabled: false, coverageReady: false, reason: 'gate-disabled' }, lastAutomaticResult: null }, demo: true });
    if (key === 'storage-status') return json({ measuredAt: new Date().toISOString(), databaseMode: 'demo（合成容量）', database: { name: 'demo_live', bytes: 2 * 1024 ** 3 }, archive: { name: 'demo_archive', bytes: 12 * 1024 ** 3, verifiedRuns: 64 }, files: [], volumes: [{ label: '模擬資料磁碟', totalBytes: 500 * 1024 ** 3, freeBytes: 200 * 1024 ** 3, warning: false, critical: false }, { label: '模擬封存磁碟', totalBytes: 2 * 1024 ** 4, freeBytes: 1.4 * 1024 ** 4, warning: false, critical: false }], warnings: ['展示容量為合成值，不是這台電腦的實際磁碟量測。'] });
    ...
    if (key === 'fg-monthly/warehouse-stock/recheck') { const row = data.source.inventory_lots.find((item) => item.sourceRecordId === params.get('recordId')); if (!row) throw new DemoError('找不到合成庫存批號。', 404); const stock = { stockPc: row.stockPc, stockKg: row.stockKg, unitWeightG: null, expectedStockPc: row.stockPc, stockPcDiff: 0, stockPcDiffPct: 0, quantityAnomaly: false }; return json({ recordId: row.sourceRecordId, sourceUrl: `/demo-record?type=inventory_lot&id=${row.sourceRecordId}`, snapshot: stock, live: stock, sourceCorrected: false, mrpRunNeedsRefresh: false }); }
    ...
    if (['diag', 'debug-source'].includes(key)) return json({ demo: true, synthetic: true, databaseConnections: 0, sourceConnections: 0, revision: state().revision, message: '僅本機合成資料，正式維運程式保留在 reference，不會執行。' });
