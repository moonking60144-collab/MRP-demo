...
export async function GET(request: NextRequest, context: Context) {
  try {
    ...
    const sql = !archiveId && postgresDemoEnabled() && !['plan-management', 'production-plans', 'outsource-price-change', 'diag', 'debug-source'].includes(key) && path[0] !== 'production-plans';
    ...
    if (['diag', 'debug-source'].includes(key)) return reportJson({ demo: true, synthetic: true, ...(sql ? { storage: 'postgresql' } : { databaseConnections: 0 }), sourceConnections: 0, revision: state().revision, message: '僅本機合成資料，正式維運程式保留在 reference，不會執行。' });
